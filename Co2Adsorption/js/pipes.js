...
export function checkAndStartMFCFlow(draw) {
    ...
    if (tankNum && tankNum !== 'outlet') { // Ensure a tank (1, 2, or 3) is selected
        ...
        if (state.getValveState(tankValveId)?.isOpen && state.getValveState(pressureValveId)?.isOpen) {
            ...
            let color, opacity;
            switch (tankNum) {
                case '1': color = '#ff0000'; opacity = 0.9; break;
                case '2': color = '#ff0000'; opacity = 0.5; break;
                case '3': color = 'blue'; opacity = 0.5; break;
                default: color = 'grey'; opacity = 1.0; // Should not happen
            }
