// js/simulation.js
import { yCO2_out } from './calc.js'; // Assuming calc.js is in the same directory or adjust path
...
export function startMoleFractionCalculation(tankNum) {
    ...
    let initialTimeOffset = 0; // Time offset to continue from previous state
    ...
    const P_bar = state.getGaugeValue(pressureGaugeId, 1.0); // Get pressure in bar, default 1.0 bar
    const P_MPa = P_bar * 0.1; // Convert bar to MPa for the function
    ...
    console.log('Calculation Params:', { tankNum, y, P_bar, P_MPa, m_controller_mg_min, m_g_s, T, desorbing: state.getDesorbing(), timeOfDesorption: state.getTimeOfDesorption(), initialTimeOffset });
    ...
    const timerId = setInterval(() => {
        ...
        const outletMoleFraction = yCO2_out({
            t: t,
        ...
        updateCO2AnalyzerDisplay(outletMoleFraction);
        ...
        if (Math.floor(t) % 5 === 0 && Math.abs(t - Math.floor(t)) < (config.timeStep * config.simulationSpeedMultiplier / 2)) { // Log roughly every 5s
             console.log(`Sim Time: ${t.toFixed(2)}s, Outlet CO2: ${outletMoleFraction.toFixed(4)}, Desorbing: ${state.getDesorbing()}, T_desorp: ${state.getTimeOfDesorption().toFixed(2)}s`);
        }
